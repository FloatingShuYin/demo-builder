 ...
 * @version $Id$
 */
var Tools, _, _buildCss, _buildMap, _cssDistPath, _cssMapPath, _cssPath, _env, _hashLen, _imgMapPath, _mapPath, _staticPath, _stream, color, cssCtl, fs, gulp, gutil, imgPathReg, mincss, path, plumber, setting;
...
imgPathReg = _env === "dev" ? "../img/" : _staticPath + "/img/";
...
_stream = function(files, cb, cb2) {
  ...
  return gulp.src(files).pipe(plumber({
    ...
    _source = String(source.contents).replace(_cssBgReg, function(str, map) {
      var key, val;
      if (map.indexOf('fonts/') !== -1 || map.indexOf('font/') !== -1 || map.indexOf('#') !== -1 || map.indexOf('data:') !== -1 || map.indexOf('about:') !== -1) {
        return str;
      } else {
        key = map.replace('../img/', '').replace(/(^\'|\")|(\'|\"$)/g, '');
        val = imgPathReg + (_.has(imgMap, key) && _env !== 'dev' ? imgMap[key].distname : key + '?=t' + String(new Date().getTime()).substr(0, 8));
        console.log(map + "--> " + val);
